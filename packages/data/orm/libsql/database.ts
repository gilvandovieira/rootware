...
/**
 * Minimal libSQL client surface used by the ORM adapter. The bundled
...
/**
 * Opens a libSQL client with the bundled `@libsql/client` driver. The driver is
 * imported lazily (a dynamic import), so importing `@rootware/orm/libsql` and
 * running its fake-backed tests pulls in no npm dependency; only a real connect
 * needs `--allow-net`/`--allow-env` (and the npm package).
 */
