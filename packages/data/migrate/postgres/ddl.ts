/**
 * Pure PostgreSQL DDL generation from `@rootware/schema` snapshots.
 *
...
import { planSchemaChanges, type SchemaChange } from "../core/mod.ts";
...
export function generatePostgresColumnType(type: RootwareColumnType): string {
  const base = pgBaseType(type);
  return type.array === true ? `${base}[]` : base;
}

function pgBaseType(type: RootwareColumnType): string {
...
function renderPgDefault(value: RootwareColumnDefault): string {
