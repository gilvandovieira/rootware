...
import { createSqliteMigrationHistoryStore } from "../sqlite/history.ts";

import type { LibsqlConnectionOptions, LibsqlLikeClient } from "./database.ts";
import { openLibsqlClient } from "./database.ts";
import { createLibsqlMigrationDriver } from "./driver.ts";
import { createLibsqlExecutor, type SqlExecutor } from "./executor.ts";
...
export async function createLibsqlMigrator(
  ...
  // libSQL speaks SQLite SQL, so the SQLite history store applies verbatim.
  ...
  return new RootwareLibsqlMigrator({
    driver,
    store,
...
class RootwareLibsqlMigrator implements LibsqlMigrator {
