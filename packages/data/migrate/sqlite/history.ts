...
/** Default SQLite table used to store applied migration history. */
export const DEFAULT_SQLITE_MIGRATION_TABLE = "rootware_migrations";
...
/** Creates a migration history store persisted in a SQLite table. */
export function createSqliteMigrationHistoryStore(
  options: SqliteMigrationHistoryStoreOptions,
): MigrationStore {
  const executor = options.executor;
  const tableName = quoteIdentifier(
    options.tableName ?? DEFAULT_SQLITE_MIGRATION_TABLE,
  );
  let ensured = false;
...
function quoteIdentifier(path: string): string {
  if (typeof path !== "string" || path.trim().length === 0) {
    throw new MigrationError("SQLite history table name is required", {
  ...
  return path.trim().split(".").map((identifier) => {
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(identifier)) {
      throw new MigrationError("SQLite identifier is invalid", {
        code: "MIGRATION_INVALID",
        status: 400,
        details: { identifier },
      });
