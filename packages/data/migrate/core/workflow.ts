 ...
 *
 * The filesystem is an injected interface (mirroring `@rootware/storage`'s
 * `StorageFileSystem`), so the writer/reader are unit-testable with an in-memory
 * fake; the default Deno-backed implementation needs `--allow-read`/
 * `--allow-write` and is exercised by the integration suite. The config, drift,
 * and file-name logic are pure.
 */
...
export interface GeneratedMigrationFile {
  ...
  readonly sql: string;
  readonly snapshot: RootwareSchemaSnapshot;
}
...
/** Validated migration configuration (`rootware.migrate.ts`). */
export interface MigrateConfig {
  readonly dir: string;
  readonly dialect?: RootwareDialectName;
  /** Prebuilt snapshot from the app (via `@rootware/orm`); omit for SQL-first. */
  readonly snapshot?: RootwareSchemaSnapshot;
...
export interface DriftCheckInput {
  /** The app's current schema snapshot (built via `@rootware/orm`). */
  readonly currentSnapshot?: RootwareSchemaSnapshot;
  /** The snapshot captured by the newest generated migration, if any. */
  readonly latestSnapshot?: RootwareSchemaSnapshot;
